...
from src.chunking import ChunkingStrategyComparator, CustomChunker
...
def chunk_documents(docs: list[Document], chunk_size: int = 1500) -> list[Document]:
    ...
    """
    chunker = CustomChunker(max_size=chunk_size, overlap=chunk_size // 5)
...
def save_index(store: EmbeddingStore, path: str) -> None:
    ...
    # Get records from ChromaDB if available, otherwise from in-memory store
    records = []
    if store._use_chroma and store._collection is not None:
        # Fetch all records from ChromaDB (include embeddings explicitly)
        all_data = store._collection.get(include=["embeddings", "documents", "metadatas"])
...
def load_index(path: str, embedder: Callable[[str], list[float]]) -> EmbeddingStore:
    ...
    if store._use_chroma and store._collection is not None:
        # Populate ChromaDB from loaded records
        ids = [rec.get("id", str(i)) for i, rec in enumerate(records)]
        documents = [rec.get("content", "") for rec in records]
        embeddings = [rec.get("embedding", []) for rec in records]
    ...
    print(f"  Loaded {len(records)} chunks ← {path}")
    return store
...
def build_index() -> int:
    ...
    print(f"  {len(docs)} docs → {len(chunks)} chunks (CustomChunker, max_size=1500, overlap=300)")
